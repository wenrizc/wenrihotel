...
### 1.1 核心机制：QoS (Quality of Service) 与 Prefetch Count
...
### 2.2 机制二：Broker 内部的流控机制 (Flow Control)
...
- 追问：RabbitMQ是如何实现限流的核心流程或关键点是什么？
  - 答：核心结论是：消费端限流的目的是为了保护消费者，防止其被无法处理的大量消息所压垮。展开时可按“消费端限流、生产端限流”组织，先概述再逐点展开，保证结构完整。其中消费端限流侧重消费端限流的目的是为了保护消费者，防止其被无法处理的大量消息所压垮，生产端限流侧重当生产者发送消息的速度远超消费者处理速度时，会导致消息在队列中大量积压，最终可能耗尽 Broker 的内存或磁盘资源。生产端限流就是为了应对这种情况。回答时要体现步骤、关键点与适用场景，必要时补充示例或对比。
- 易错点：RabbitMQ是如何实现限流中最容易混淆或踩坑的点是什么？
  - 答：常见易错点是只给结论不讲依据、边界条件与前提不清。比如消费端限流中提到：消费端限流的目的是为了保护消费者，防止其被无法处理的大量消息所压垮。生产端限流中还提到：当生产者发送消息的速度远超消费者处理速度时，会导致消息在队列中大量积压，最终可能耗尽 Broker 的内存或磁盘资源。生产端限流就是为了应对这种情况。这些细节很容易被忽视。回答时应明确边界、关键步骤与适用场景，并用实例或对比验证。
