## 1. 生产阶段：确保消息成功发送到 RabbitMQ

在生产阶段，核心是确保生产者发送的消息能够被 RabbitMQ 服务端成功接收并处理。

### 1.1 方案一：AMQP 事务机制 (Transactions)
...
### 1.2 方案二：发送方确认机制 (Publisher Confirms)
...
### 3.1 消费者确认机制 (Consumer Acknowledgements)
...
### 4.2 镜像队列 (Mirrored Queues) / Quorum 队列
