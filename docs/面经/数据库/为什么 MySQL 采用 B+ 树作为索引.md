...
3.  自平衡二叉树 (Self-Balancing Binary Search Tree, 如 AVL 树、红黑树)
    - 优点：通过特定的约束条件（如 AVL 树中任意节点的左右子树高度差不超过 1）来维持树的平衡，避免了退化成链表的情况，确保查询时间复杂度稳定在 O(logn)。
...
2.  插入和删除效率更高：B+ 树有大量的冗余节点。在删除节点时，通常可以直接从叶子节点中进行，对非叶子节点的影响较小，不需要像 B 树那样进行复杂的树结构变形。B+ 树每次都到叶子，路径更稳定（都走同样层数），更利于实现和缓存/磁盘访问模式优化。
