...
- 追问：为什么 MySQL 采用 B+ 树作为索引的核心流程或关键点是什么？
  - 答：核心结论是：B+ 树是对 B 树的优化和升级，也是 MySQL InnoDB 存储引擎最终采用的索引结构。展开时可按“数据结构的演进、B+ 树、MySQL (InnoDB) 中的 B+ 树特性”组织，先概述再逐点展开，保证结构完整。其中数据结构的演进侧重二分查找与数组，B+ 树侧重B+ 树是对 B 树的优化和升级，也是 MySQL InnoDB 存储引擎最终采用的索引结构。回答时要体现步骤、关键点与适用场景，必要时补充示例或对比。
- 易错点：为什么 MySQL 采用 B+ 树作为索引中最容易混淆或踩坑的点是什么？
  - 答：常见易错点是只给结论不讲依据、边界条件与前提不清。比如数据结构的演进中提到：二分查找与数组。B+ 树中还提到：B+ 树是对 B 树的优化和升级，也是 MySQL InnoDB 存储引擎最终采用的索引结构。这些细节很容易被忽视。回答时应明确边界、关键步骤与适用场景，并用实例或对比验证。
