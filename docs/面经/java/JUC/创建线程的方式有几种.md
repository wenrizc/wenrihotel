...
## 4. 实现 `Callable` 接口（配合 `FutureTask` 或线程池）
...
- 追问：创建线程的方式有几种的核心流程或关键点是什么？
  - 答：核心结论是：这是最直接的一种方式，通过创建一个继承自 `Thread` 类的子类，并重写其 `run()` 方法来定义线程需要执行的任务。展开时可按“继承 `Thread` 类、实现 `Runnable` 接口、实现 `Callable` 接口（配合 `FutureTask` 或线程池）”组织，先概述再逐点展开，保证结构完整。其中继承 `Thread` 类侧重这是最直接的一种方式，通过创建一个继承自 `Thread` 类的子类，并重写其 `run()` 方法来定义线程需要执行的任务，实现 `Runnable` 接口侧重这种方式下，线程的执行任务（`Runnable`）与线程的创建和管理（`Thread`）是分离的。回答时要体现步骤、关键点与适用场景，必要时补充示例或对比。
- 易错点：创建线程的方式有几种中最容易混淆或踩坑的点是什么？
  - 答：常见易错点是只给结论不讲依据、边界条件与前提不清。比如继承 `Thread` 类中提到：这是最直接的一种方式，通过创建一个继承自 `Thread` 类的子类，并重写其 `run()` 方法来定义线程需要执行的任务。实现 `Runnable` 接口中还提到：这种方式下，线程的执行任务（`Runnable`）与线程的创建和管理（`Thread`）是分离的。这些细节很容易被忽视。回答时应明确边界、关键步骤与适用场景，并用实例或对比验证。
