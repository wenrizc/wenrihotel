...
- 追问：JDK8新特性的核心流程或关键点是什么？
  - 答：核心结论是：Java 8 最引人注目的新特性无疑是 **Lambda 表达式**。它允许我们将函数作为方法的参数进行传递，这种将代码像数据一样传递的方式，是函数式编程的核心思想。展开时可按“Lambda 表达式与函数式编程、Stream API：高效的集合处理、全新的日期和时间 API”组织，先概述再逐点展开，保证结构完整。其中Lambda 表达式与函数式编程侧重Java 8 最引人注目的新特性无疑是 **Lambda 表达式**。它允许我们将函数作为方法的参数进行传递，这种将代码像数据一样传递的方式，是函数式编程的核心思想，Stream API：高效的集合处理侧重与 Lambda 表达式相辅相成的是全新的 **Stream API** (`java.util.stream`)。它提供了一种声明式的方式来处理数据集合，让开发者可以像书写 SQL 查询语句一样对集合进行复杂的操作。回答时要体现步骤、关键点与适用场景，必要时补充示例或对比。
- 易错点：JDK8新特性中最容易混淆或踩坑的点是什么？
  - 答：常见易错点是只给结论不讲依据、边界条件与前提不清。比如Lambda 表达式与函数式编程中提到：Java 8 最引人注目的新特性无疑是 **Lambda 表达式**。它允许我们将函数作为方法的参数进行传递，这种将代码像数据一样传递的方式，是函数式编程的核心思想。Stream API：高效的集合处理中还提到：与 Lambda 表达式相辅相成的是全新的 **Stream API** (`java.util.stream`)。它提供了一种声明式的方式来处理数据集合，让开发者可以像书写 SQL 查询语句一样对集合进行复杂的操作。这些细节很容易被忽视。回答时应明确边界、关键步骤与适用场景，并用实例或对比验证。
