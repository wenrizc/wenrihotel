...
## 4. 监视器锁与 `wait()` / `notify()` / `notifyAll()` 的交互
...
- 追问：JVM监视器锁是什么的核心流程或关键点是什么？
  - 答：核心结论是：每个Java对象在JVM层面都与一个监视器（Monitor）相关联。这个监视器主要由以下三个部分组成：。展开时可按“监视器锁的核心组成部分、监视器锁的工作流程、监视器锁与 `wait()` / `notify()` / `notifyAll()` 的交互”组织，先概述再逐点展开，保证结构完整。其中监视器锁的核心组成部分侧重每个Java对象在JVM层面都与一个监视器（Monitor）相关联。这个监视器主要由以下三个部分组成：，监视器锁的工作流程侧重当一个线程遇到`synchronized`关键字时，其工作流程如下：。回答时要体现步骤、关键点与适用场景，必要时补充示例或对比。
- 易错点：JVM监视器锁是什么中最容易混淆或踩坑的点是什么？
  - 答：常见易错点是忽略步骤顺序或前置条件。比如监视器锁的核心组成部分中提到：每个Java对象在JVM层面都与一个监视器（Monitor）相关联。这个监视器主要由以下三个部分组成：。监视器锁的工作流程中还提到：当一个线程遇到`synchronized`关键字时，其工作流程如下：。这些细节很容易被忽视。回答时应明确边界、关键步骤与适用场景，并用实例或对比验证。
