...
- 追问：javac编译过程的核心流程或关键点是什么？
  - 答：核心结论是：`javac` 编译器负责将包含 Java 源代码的 `.java` 文件转换成平台无关的字节码文件。这些字节码文件可以被任何安装了 Java 虚拟机（JVM）的设备执行，实现了 Java “一次编写，到处运行” (Write Once, Run Anywhere - WORA) 的特性。展开时可按“`javac` 的作用、`javac` 编译过程的阶段”组织，先概述再逐点展开，保证结构完整。其中`javac` 的作用侧重`javac` 编译器负责将包含 Java 源代码的 `.java` 文件转换成平台无关的字节码文件。这些字节码文件可以被任何安装了 Java 虚拟机（JVM）的设备执行，实现了 Java “一次编写，到处运行” (Write Once, Run Anywhere - WORA) 的特性，`javac` 编译过程的阶段侧重`javac` 的编译过程通常可以分解为以下几个主要阶段：。回答时要体现步骤、关键点与适用场景，必要时补充示例或对比。
- 易错点：javac编译过程中最容易混淆或踩坑的点是什么？
  - 答：常见易错点是只给结论不讲依据、边界条件与前提不清。比如`javac` 的作用中提到：`javac` 编译器负责将包含 Java 源代码的 `.java` 文件转换成平台无关的字节码文件。这些字节码文件可以被任何安装了 Java 虚拟机（JVM）的设备执行，实现了 Java “一次编写，到处运行” (Write Once, Run Anywhere - WORA) 的特性。`javac` 编译过程的阶段中还提到：`javac` 的编译过程通常可以分解为以下几个主要阶段：。这些细节很容易被忽视。回答时应明确边界、关键步骤与适用场景，并用实例或对比验证。
