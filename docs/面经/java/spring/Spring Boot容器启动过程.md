...
- 易错点：Spring Boot容器启动过程中最容易混淆或踩坑的点是什么？
  - 答：常见易错点是忽略步骤顺序或前置条件。比如启动入口：`SpringApplication.run()`中提到：每个 Spring Boot 项目都有一个主启动类，其中包含一个 `main` 方法，该方法通常只有一行代码：`SpringApplication.run(MyApplication.class, args);`。这行代码是整个启动流程的起点。`SpringApplication` 对象的初始化中还提到：在 `SpringApplication` 的构造函数中，会执行一系列初始化操作：。这些细节很容易被忽视。回答时应明确边界、关键步骤与适用场景，并用实例或对比验证。
