...
- 可用性与延迟：以业务 SLO 为准，重点看 P99 延迟、超时率与事件循环延迟趋势。
- 容量：关注 `used_memory` 与 `maxmemory` 的距离，以及 `evicted_keys` 是否持续增长（意味着开始淘汰）。
- 复制：关注 `master_link_status`、复制延迟与 `repl_backlog` 相关指标，避免“看着活着但数据落后很多”。

## 9. 总结

- `PING`/端口只能证明“活着”，要用 `INFO`、`SLOWLOG`、`LATENCY DOCTOR` 判断“是否健康”。
- 集群/主从场景要额外看角色与复制链路，避免读到落后的数据或写入错误节点。
