...
## 3. `BGSAVE` 命令 (非阻塞，但 `fork` 阶段有短暂阻塞)
...
- 追问：Redis的RDB快照记录时会阻塞吗的核心流程或关键点是什么？
  - 答：核心结论是：`SAVE` 命令会**完全阻塞** Redis 服务器进程。展开时可按“`SAVE` 命令 (阻塞)、`BGSAVE` 命令 (非阻塞，但 `fork` 阶段有短暂阻塞)”组织，先概述再逐点展开，保证结构完整。其中`SAVE` 命令 (阻塞)侧重`SAVE` 命令会**完全阻塞** Redis 服务器进程，`BGSAVE` 命令 (非阻塞，但 `fork` 阶段有短暂阻塞)侧重`BGSAVE` (Background Save) 命令是 Redis 默认和推荐的 RDB 快照方式，它旨在避免阻塞 Redis 主线程。回答时要体现步骤、关键点与适用场景，必要时补充示例或对比。
- 易错点：Redis的RDB快照记录时会阻塞吗中最容易混淆或踩坑的点是什么？
  - 答：常见易错点是只给结论不讲依据、边界条件与前提不清。比如`SAVE` 命令 (阻塞)中提到：`SAVE` 命令会**完全阻塞** Redis 服务器进程。`BGSAVE` 命令 (非阻塞，但 `fork` 阶段有短暂阻塞)中还提到：`BGSAVE` (Background Save) 命令是 Redis 默认和推荐的 RDB 快照方式，它旨在避免阻塞 Redis 主线程。这些细节很容易被忽视。回答时应明确边界、关键步骤与适用场景，并用实例或对比验证。
