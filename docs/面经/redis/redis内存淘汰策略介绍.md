...
### 2.3 仅针对设置了过期时间的键 (volatile) 的淘汰策略
...
- 追问：Redis内存淘汰策略介绍的核心流程或关键点是什么？
  - 答：核心结论是：在 Redis 中，通过以下两个配置指令来控制内存淘汰：。展开时可按“**如何配置内存淘汰策略**、**八种内存淘汰策略**、**如何选择合适的淘汰策略**”组织，先概述再逐点展开，保证结构完整。其中**如何配置内存淘汰策略**侧重在 Redis 中，通过以下两个配置指令来控制内存淘汰：，**八种内存淘汰策略**侧重Redis 提供了八种不同的内存淘汰策略，可以分为两大类：针对所有键和仅针对设置了过期时间的键。回答时要体现步骤、关键点与适用场景，必要时补充示例或对比。
- 易错点：Redis内存淘汰策略介绍中最容易混淆或踩坑的点是什么？
  - 答：常见易错点是只给结论不讲依据、边界条件与前提不清。比如**如何配置内存淘汰策略**中提到：在 Redis 中，通过以下两个配置指令来控制内存淘汰：。**八种内存淘汰策略**中还提到：Redis 提供了八种不同的内存淘汰策略，可以分为两大类：针对所有键和仅针对设置了过期时间的键。这些细节很容易被忽视。回答时应明确边界、关键步骤与适用场景，并用实例或对比验证。
