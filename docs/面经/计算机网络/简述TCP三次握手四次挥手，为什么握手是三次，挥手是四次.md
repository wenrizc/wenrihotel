...
- 追问：简述TCP三次握手四次挥手，为什么握手是三次，挥手是四次的核心流程或关键点是什么？
  - 答：核心结论是：三次握手的核心目标是**同步双方的初始序列号（ISN）并确认双方的收发能力都正常**。可以把它想象成一个打电话确认信号的过程。展开时可按“TCP三次握手（建立连接）、TCP四次挥手（断开连接）、为什么握手是三次，而挥手是四次？”组织，先概述再逐点展开，保证结构完整。其中TCP三次握手（建立连接）侧重三次握手的核心目标是**同步双方的初始序列号（ISN）并确认双方的收发能力都正常**。可以把它想象成一个打电话确认信号的过程，TCP四次挥手（断开连接）侧重四次挥手的核心目标是**确保双方都没有数据要发送了，然后安全地关闭连接**。回答时要体现步骤、关键点与适用场景，必要时补充示例或对比。
- 易错点：简述TCP三次握手四次挥手，为什么握手是三次，挥手是四次中最容易混淆或踩坑的点是什么？
  - 答：常见易错点是只给结论不讲依据、边界条件与前提不清。比如TCP三次握手（建立连接）中提到：三次握手的核心目标是**同步双方的初始序列号（ISN）并确认双方的收发能力都正常**。可以把它想象成一个打电话确认信号的过程。TCP四次挥手（断开连接）中还提到：四次挥手的核心目标是**确保双方都没有数据要发送了，然后安全地关闭连接**。这些细节很容易被忽视。回答时应明确边界、关键步骤与适用场景，并用实例或对比验证。
