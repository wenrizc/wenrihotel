...
## 3. 确认应答机制 (Acknowledgements, ACKs)
...
## 4. 超时重传 (Timeout and Retransmission)
...
- 追问：TCP是如何实现可靠传输的核心流程或关键点是什么？
  - 答：核心结论是：这是TCP实现有序传输和数据完整性检查的基础。展开时可按“序列号 (Sequence Numbers)、确认应答机制 (Acknowledgements, ACKs)、超时重传 (Timeout and Retransmission)”组织，先概述再逐点展开，保证结构完整。其中序列号 (Sequence Numbers)侧重这是TCP实现有序传输和数据完整性检查的基础，确认应答机制 (Acknowledgements, ACKs)侧重这个机制用来确认数据是否被对方成功接收。回答时要体现步骤、关键点与适用场景，必要时补充示例或对比。
- 易错点：TCP是如何实现可靠传输中最容易混淆或踩坑的点是什么？
  - 答：常见易错点是只给结论不讲依据、边界条件与前提不清。比如序列号 (Sequence Numbers)中提到：这是TCP实现有序传输和数据完整性检查的基础。确认应答机制 (Acknowledgements, ACKs)中还提到：这个机制用来确认数据是否被对方成功接收。这些细节很容易被忽视。回答时应明确边界、关键步骤与适用场景，并用实例或对比验证。
